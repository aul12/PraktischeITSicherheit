...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \visible<3->{\item Code can be written in trusted or untrusted functions}
        \visible<4->{\item trusted functions run inside the enclave}
        \visible<5->{\item Application consists of its own code, own data and the enclave}
        \visible<6->{\item Enclave is launched by the application}
        \visible<7->{\item Enclave Code can be run from the application by call to the enclave entry point}
        \visible<8>{\item Entry Point is a execution of the enclave and is pre-defined during compilation}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \visible<1->{\item Trusted functions are implemented inside the enclave}
        \visible<2->{\item Only security critical functions should be implement in the enclave, e.g Password Reads}
        \visible<3->{\item Enclave Memory could only be read from trusted functions}
        \visible<3->{\item Trusted functions should not directly pass a pointer to enclave memory}
    \end{itemize}
\end{frame}
