...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \visible<2->{\item Secure part is called ``enclave``}
...
\incfuck[0.8\textwidth]{AppOverview}{1}{Source: \url{}}{Overview}
\incfuck[0.8\textwidth]{AppOverview}{2}{Source: \url{}}{Overview}
\incfuck[0.8\textwidth]{AppOverview}{3}{Source: \url{}}{Overview}
\incfuck[0.8\textwidth]{AppOverview}{4}{Source: \url{}}{Overview}
\incfuck[0.8\textwidth]{AppOverview}{5}{Source: \url{}}{Overview}
\incfuck[0.8\textwidth]{AppOverview}{6}{Source: \url{}}{Overview}
\incfuck[0.8\textwidth]{AppOverview}{7}{Source: \url{}}{Overview}
\incfuck[0.8\textwidth]{AppOverview}{8}{Source: \url{}}{Overview}

\begin{frame}
    \frametitle{Overview}
    \begin{itemize}
        \visible<1->{\item Enclave contains also its own code and data}
        \visible<2->{\item Enclave is backed by encrypted memory pages which are reserved inside the RAM}
        \visible<3->{\item The OS itself can not read data from the enclave, because encryption is on hardware layer}
        \visible<4->{\item The encrypted memory pages can only be accessed from the enclave, not from the application}
        \visible<5->{\item Enclave entry point (address pointer) is pre-defined during compilation and won't change}
        \visible<6->{\item Enclaves can access their application memory, but not the other way arround}
        \visible<7>{\item Code can be written in trusted or untrusted functions}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \visible<1->{\item Trusted functions are linked to application}
        \visible<2->{\item Trusted are implemented inside the enclave}
        \visible<3>{\item Trusted functions will not directly pass a pointer to memory}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \visible<5->{\item CPU has an extra enclave Mode to call some of these instructions}
        \visible<6->{\item Enclave mode needs a context switch and is only for enclave handling}
        \visible<7->{\item Example for Instructions are EENTRY or EEXIT}
        \visible<8>{\item The processor can enter or exit the enclave with this two instructions in enclave mode}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \visible<3>{\item SGX enclave control structure (SECS) contains meta-data like hash value or the enclave size}
    \end{itemize}
\end{frame}
